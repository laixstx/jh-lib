...
//左边菜单栏点击和搜索点击发送的ajax请求
  fetchList() {
    ...
    //   this.setState({loading: true}, () => {
    //     this.props.fetchFunc(this.searchData).then((dataList: any) => {
    //
    //       $this.setState({
                ...
                >
                  {
                    ...
                    map(typeList, (v) => {
                      let defaultProps = this.props.defaultProps;
                      return <Menu.Item style={{margin: 0}} key={defaultProps?v[defaultProps]:v.code}>{v.name}</Menu.Item>
                    })
                  }
                </Menu>
              </Col>
