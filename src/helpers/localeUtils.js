...
import { recursiveFind } from '../array';
...
// zh_cn,formatMessage,setLocale,suffix
/**
 * 判断 key 是否有配置国际化
 * @param key
 */
...
export function changeLocale(locale) {
  let actionObj = {
    type: 'global/updateRandomKey',
    payload: locale,
  };

  if (window.parent && window.parent.dvaDispatch) {
    window.parent.setLocale(locale, false);
    window.parent.localStorage.removeItem(LOCALE_FORMAT_KEY);
    window.parent.dvaDispatch(actionObj);
  }
...
export function formatMsgByCn(formatMessage=null,cn, values = {}) {
